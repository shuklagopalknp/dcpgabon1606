<?php 
class PP_Credit_Scholar_Approval_Analyst extends Admin_Controller 
{
	...
	*/
	...
	public function credit_scholar_loan(){
		...
		$this->data['loandetails']=$this->RiskAnalyst_Loans_Model->get_Allapplication_form_credit_scholar($branchid);
		$this->data['headagency']=$this->PP_Credit_Scholar_Model->get_headagency($this->data['loandetails'][0]['branch_id']);
		$user_id = $this->session->userdata('id');
		$is_admin = ($user_id == 6) ? true :false;
		$this->data['is_admin'] = $is_admin;
		if($this->session->userdata('role')==='6'){			
			$this->render_template2('analyst/creditscholar_loan.php', $this->data);
		}else{			
	...
	public function customer_details()
	{
		...
		$this->data['appformD']=$this->RiskAnalyst_Loans_Model->get_customar_details_credit_scholar($id);
		...
		if($this->session->userdata('role')==='6'){			
			$this->render_template2('analyst/credit_scholar_customer_details', $this->data);
		}else{			
			$this->data['title'] = 'DCP | Access Denied';
			$this->simple_template('access_denied', $this->data);          
		}				
	...
	public function amortizationview()
    {
		...
		$this->data['is_admin'] = $is_admin;
		if($this->session->userdata('role')==='6'){			
			$this->render_template2('analyst/cscholar_amortization_view', $this->data);  
		}else{			
			$this->data['title'] = 'DCP | Access Denied';
			$this->simple_template('access_denied', $this->data);          
		}		
	...
	public function searchloanstatus(){
		...
		$recordcheck=$this->RiskAnalyst_Loans_Model->checkloanstatuslist(trim($this->input->post('filter')));
		...
		if(!empty($recordcheck)){
			foreach ($recordcheck as $key) {
				...
				if(timeAgo($time_ago) >= 48){
					if($key['dcpranalyst_status']=="Process" || $key['dcpranalyst_status']=="Demande de retraitement"){
				...
				$html .="</td>			
					...
					<td class=\"sorting_1\">
						<a href=\"".base_url('PP_Credit_Scholar_Approval_Analyst/customer_details/').$key['cap_id']."\" class=\"table-link\">
							<span class=\"fa-stack\">
  ...
  */
	public function sendMailToHigherOfficial($loanId)
	{
		...
		$subject = "Analyst DCPR Avis favorable for a loan CREDIT SCOLAIRE";
