...
class PP_Caution_Scolare_Loans_DirectorEngagements extends Admin_Controller 
{
	...
	*/
	public function index(){
		...
		$this->data['loandetails']=$this->DirectorEngagements_Loans_Model->get_caution_scolare_Loans();
		$this->data['loanrange']=$this->PP_Caution_Scolare_Loans_Model->get_pprange();
		$user_id = $this->session->userdata('id');
		$is_admin = ($user_id !=1) ? true :false;
		$this->data['is_admin'] = $is_admin;
		if($this->session->userdata('role')==='10'){			
			$this->render_template2('director_engagements/Caution_Scolare_loan', $this->data);
		}else{			
	...
	public function customer_details()
	{
		...
		$this->data['appformD']=$this->DirectorEngagements_Loans_Model->get_customar_details_caution_scolare($id);
		...
		if($this->session->userdata('role')==='10'){			
			$this->render_template2('director_engagements/caution_scolaire_customer_details', $this->data);
		}else{			
			$this->data['title'] = 'DCP | Access Denied';
			$this->simple_template('access_denied', $this->data);          
		}				
    ...
    public function amortization_loan(){
		...
		$this->data['is_admin'] = $is_admin;		
		if($this->session->userdata('role')==='10'){			
			$this->render_template2('director_engagements/amortization_caution_scolare', $this->data);
		}else{			
			$this->data['title'] = 'DCP | Access Denied';
			$this->simple_template('access_denied', $this->data);          
		}		
    ...
    public function amortizationview()
    {
		...
		if($this->session->userdata('role')==='10'){			
			$this->render_template2('director_engagements/amortization_view', $this->data); 
		}else{			
			$this->data['title'] = 'DCP | Access Denied';
			$this->simple_template('access_denied', $this->data);          
		}		   	
	...
	function temporary_generate_amortization_pdf($segmentid){		
		...
		return  $this->load->view('director_engagements/caution_scolaire_pdf_generate',$this->data,true);
	}
	function generate_amortization_pdf1($segmentid){		
		...
		return  $this->load->view('director_engagements/amortization_pdf_generate',$this->data,true);
	}
	...
	public function searchloanstatus(){		
		$recordcheck=$this->DirectorEngagements_Loans_Model->filter_Caution_Scolare_loan(trim($this->input->post('filter')));		
		$html=null;
		if(!empty($recordcheck)){
			foreach ($recordcheck as $key) {
				...
				<td>";						
					...
					<td class=\"sorting_1\">
						<a href=\"".base_url('PP_Caution_Scolare_Loans_DirectorEngagements/customer_details/').$key['cl_aid']."\" class=\"table-link\">
							<span class=\"fa-stack\">
	...
	public function sendMailToHigherOfficial($cl_aid)
	{
		...
		# Get role id
		$subAdminRoleId = 11; // Account Manager
		...
		$subject = "Director Engagements Decision for a loan CAUTION SCOLAIRE";
