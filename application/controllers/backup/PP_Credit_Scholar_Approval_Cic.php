<?php 
class PP_Credit_Scholar_Approval_Cic extends Admin_Controller 
{
	...
	*/
	...
	public function credit_scholar_loan(){
		...
		$this->data['loandetails']=$this->Cic_Loans_Model->get_Allapplication_form_credit_scholar($branchid);
		$user_id = $this->session->userdata('id');
		$is_admin = ($user_id == $this->session->userdata('id')) ? true :false;
		$this->data['is_admin'] = $is_admin;
		if($this->session->userdata('role')==='8'){			
			$this->render_template2('cic/creditscholar_loan.php', $this->data);
		}else{			
	...
	public function amortizationview()
    {
		...
		$this->data['is_admin'] = $is_admin;
		if($this->session->userdata('role')==='8'){			
			$this->render_template2('cic/cscholar_amortization_view', $this->data);  
		}else{			
			$this->data['title'] = 'DCP | Access Denied';
			$this->simple_template('access_denied', $this->data);          
		}		
	...
	public function customer_details()
	{
		...
		$user_id = $this->session->userdata('id');
		$id=$this->uri->segment(3);	
		$this->data['appformD']=$this->Cic_Loans_Model->get_customar_details_credit_scholar($id);
		...
		if($this->session->userdata('role')==='8'){			
			$this->render_template2('cic/credit_scholar_customer_details', $this->data);
		}else{			
			$this->data['title'] = 'DCP | Access Denied';
			$this->simple_template('access_denied', $this->data);          
		}				
	...
	public function searchloanstatus(){
		...
		$recordcheck=$this->Cic_Loans_Model->checkloanstatuslist(trim($this->input->post('filter')));
		...
		if(!empty($recordcheck)){
			foreach ($recordcheck as $key) {
				...
				$html .="</td>			
					...
					<td class=\"sorting_1\">
						<a href=\"".base_url('PP_Credit_Scholar_Approval_Cic/customer_details/').$key['cap_id']."\" class=\"table-link\">
							<span class=\"fa-stack\">
  ...
  */
	public function sendMailToHigherOfficial($loanId)
	{
		...
		$subject = "CIC Avis favorable for a loan CREDIT SCOLAIRE";
