...
class PP_Consumer_Approval_Analyst extends Admin_Controller 
{
	public function __construct()
	{
		parent::__construct();
		$this->not_logged_in();
		$this->data['title'] = 'DCP | PP Consumer Loans';	
	...
	*/
	...
	public function consumerloan(){
		...
		$this->data['loandetails']=$this->RiskAnalyst_Loans_Model->get_Allapplication_form_consumerloan($branchid);
	...
	public function customer_details()
	{
		...
		$this->data['appformD']=$this->RiskAnalyst_Loans_Model->get_customar_details_consumerloan($id);
		...
		if($this->session->userdata('role')==='6'){			
			$this->render_template2('analyst/consumer_loan_customer_details', $this->data);
		}else{			
			$this->data['title'] = 'DCP | Access Denied';
			$this->simple_template('access_denied', $this->data);          
		}				
	...
	public function amortizationview()
    {
		...
		$this->data['is_admin'] = $is_admin;
		if($this->session->userdata('role')==='6'){			
			$this->render_template2('analyst/consumer_amortization_view', $this->data);  
		}else{			
			$this->data['title'] = 'DCP | Access Denied';
			$this->simple_template('access_denied', $this->data);          
		}		
	...
	public function comment_manager()
    {
		...
		else
		{
			...
			if($insert){
				...
				{
					if($this->input->post('loan_amount') <=15000001){
						$data = array(
						'dcpranalyst_status'=>trim($this->input->post('decision')),
						'cic_status'=>'Process',
	...
	public function searchloanstatus(){
		...
		$recordcheck=$this->RiskAnalyst_Loans_Model->filterConsumerloan(trim($this->input->post('filter')),trim($this->input->post('bid')) );
		...
		if(!empty($recordcheck)){
			foreach ($recordcheck as $key) {
				...
				$html .="</td>			
					...
					<td class=\"sorting_1\">
						<a href=\"".base_url('PP_Credit_Scholar_Approval_Analyst/customer_details/').$key['cl_aid']."\" class=\"table-link\">
							<span class=\"fa-stack\">
	...
	public function sendMailToHigherOfficial($cl_aid)
	{
		...
		if($getLoanDetails['loan_amt'] > 5000001 && $getLoanDetails['loan_amt'] <= 15000000) {
			$subAdminRoleId = 10;
		...
		$subject = "Analyst Dcpr Avis favorable for a loan CREDIT CONSO";
