...
class PP_Caution_Scolare_Loans_Cda extends Admin_Controller 
{
	...
	*/
	...
	public function index(){
		...
		$this->data['loandetails']=$this->Cda_Loans_Model->get_caution_scolare_Loans();
		$this->data['loanrange']=$this->PP_Caution_Scolare_Loans_Model->get_pprange();
		$user_id = $this->session->userdata('id');
		$is_admin = ($user_id !=1) ? true :false;
		$this->data['is_admin'] = $is_admin;
		if($this->session->userdata('role')==='9'){			
			$this->render_template2('cad/Caution_Scolare_loan', $this->data);
		}else{			
	...
	public function customer_details()
	{
		...
		$this->data['appformD']=$this->Cda_Loans_Model->get_customar_details_caution_scolare($id);
	...
	public function comment_manager()
  {
	...
	public function searchloanstatus(){		
		$recordcheck=$this->Cda_Loans_Model->filter_Caution_Scolare_loan(trim($this->input->post('filter')));		
		$html=null;
		if(!empty($recordcheck)){
			foreach ($recordcheck as $key) {
				...
				<td>";						
					...
					<td class=\"sorting_1\">
						<a href=\"".base_url('PP_Caution_Scolare_Loans_Cda/customer_details/').$key['cl_aid']."\" class=\"table-link\">
							<span class=\"fa-stack\">
  ...
  */
	public function sendMailToHigherOfficial($cl_aid)
	{
		...
		$subject = "CAD Decision for a loan CAUTION SCOLAIRE";
