...
class PP_Caution_Scolare_Loans_Operating_Director extends Admin_Controller 
{
	...
	*/
	...
	public function index(){
		...
		$this->data['loandetails']=$this->Operating_Director_Loans_Model->get_caution_scolare_Loans();
		$this->data['loanrange']=$this->PP_Caution_Scolare_Loans_Model->get_pprange();
		$user_id = $this->session->userdata('id');
		$is_admin = ($user_id !=1) ? true :false;
		$this->data['is_admin'] = $is_admin;
		if($this->session->userdata('role')==='12'){			
			$this->render_template2('operating_manager/Caution_Scolare_loan', $this->data);
		}else{			
	...
	public function customer_details()
	{
		...
		$this->data['appformD']=$this->Operating_Director_Loans_Model->get_customar_details_caution_scolare($id);
		...
		if($this->session->userdata('role')==='12'){	
		        // $this->render_template2('cad/caution_scolaire_customer_details', $this->data);		
			$this->render_template2('operating_manager/caution_scolaire_customer_details', $this->data);
		}else{			
			$this->data['title'] = 'DCP | Access Denied';
			$this->simple_template('access_denied', $this->data);          
		}				
    ...
    public function amortization_loan(){
		...
		$this->data['is_admin'] = $is_admin;		
		if($this->session->userdata('role')==='12'){			
			$this->render_template2('cad/amortization_caution_scolare', $this->data);
		}else{			
			$this->data['title'] = 'DCP | Access Denied';
			$this->simple_template('access_denied', $this->data);          
		}		
    ...
    public function amortizationview()
    {
		...
		if($this->session->userdata('role')==='11'){			
			$this->render_template2('cad/amortization_view', $this->data); 
		}else{			
			$this->data['title'] = 'DCP | Access Denied';
			$this->simple_template('access_denied', $this->data);          
		}		   	
	...
	public function confirm_disbursement(){
		...
		error_reporting(0);
		//print_r($_POST); exit;
		$getcp=$this->Operating_Director_Loans_Model->get_loanamount($this->input->post('cl_aid'));
	...
	$this->PP_Caution_Scolare_Loans_Model->updateRow('caution_scolaire_loans_applicationform', 'cl_aid',$editid, $status);
	$customerinfo=$this->Operating_Director_Loans_Model->get_PersonalInformation_cus($this->input->post('cl_aid'));
...
//echo $body; exit;
    ...
    $this->email->cc('prashanth.saride@gmail.com','info@dcpilote.com'); 
    $this->email->subject('Confirm Loans-Interaction');
	...
	public function sendMailToHigherOfficial($cl_aid)
	{
		...
		# Get role id
		$subAdminRoleId = 12; // Account Manager
		...
		$subject = "Operating Director Decision for a loan CAUTION SCOLAIRE";
