<?php 
class PP_Credit_Scholar_Approval_Cda extends Admin_Controller 
{
	...
	*/
	...
	public function credit_scholar_loan(){
		...
		$this->data['loandetails']=$this->Cda_Loans_Model->get_Allapplication_form_credit_scholar($branchid);
		$user_id = $this->session->userdata('id');
		$is_admin = ($user_id == 12) ? true :false;
		$this->data['is_admin'] = $is_admin;
		if($this->session->userdata('role')==='9'){			
			$this->render_template2('cad/creditscholar_loan.php', $this->data);
		}else{			
	...
	public function customer_details()
	{
		...
		$this->data['appformD']=$this->Cda_Loans_Model->get_customar_details_credit_scholar($id);
		...
		if($this->session->userdata('role')==='9'){			
			$this->render_template2('cad/credit_scholar_customer_details', $this->data);
		}else{			
			$this->data['title'] = 'DCP | Access Denied';
			$this->simple_template('access_denied', $this->data);          
		}				
	...
	public function amortizationview()
    {
		...
		$this->data['is_admin'] = $is_admin;
		if($this->session->userdata('role')==='9'){			
			$this->render_template2('cad/cscholar_amortization_view', $this->data);
		}else{			
			$this->data['title'] = 'DCP | Access Denied';
			$this->simple_template('access_denied', $this->data);          
		}		
	...
	public function comment_manager()
    {
		...
		if(!empty($this->input->post('filedzome'))){
			$status .='CAD Confirm Disbursement this loan |';
		}
	...
	public function searchloanstatus(){
		...
		$recordcheck=$this->Cda_Loans_Model->checkloanstatuslist(trim($this->input->post('filter')));
		...
		if(!empty($recordcheck)){
			foreach ($recordcheck as $key) {
				...
				$html .="</td>			
					...
					<td class=\"sorting_1\">
						<a href=\"".base_url('PP_Credit_Scholar_Approval_Cda/customer_details/').$key['cap_id']."\" class=\"table-link\">
							<span class=\"fa-stack\">
  ...
  */
	public function sendMailToHigherOfficial($loanId)
	{
		...
		$subject = "Head Cad Avis favorable for a loan CREDIT SCOLAIRE";
