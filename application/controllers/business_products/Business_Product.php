...
class Business_Product extends Admin_Controller
{
	...
	public function comment_manager_withdraw($loan_id){

		//print_r($_POST);

		$decision =  $this->input->post('w_decision');
		$commentstatus =  $this->input->post('w_comment');
		$role_id =  $this->session->userdata('role');
		$user_id =  $this->session->userdata('id');
		...
		$output = '';
		$this->form_validation->set_error_delimiters('<span>', '</span>');
		$this->form_validation->set_rules('w_decision','Please Select From Below', 'required');
		$this->form_validation->set_rules('w_comment','Comment', 'required');	
		...
		{
			if($decision =="Avis Favorable" || $decision =="Avis défavorable")
			{
				$decision_data =  array(
									'loan_id' => $loan_id,
									'user_id' => $user_id,
			...
			// Current Person of Workflow 
			$current_userworkflow =  $this->db->get_where('tbl_commune_withdrawal_approval',array('loan_id' => $loan_id,'assigned_roles' => $role_id,'status' => 1))->row_array();

			if($this->Common_model->updateRow('tbl_commune_withdrawal_approval','id', $current_userworkflow['id'], $decision_data)){
				$flag = 1;
			}
			...
			if( $decision != "Avis défavorable")
			{
				// Next Person of Workflow 
				$this->db->limit(1);
				$next_userworkflow =  $this->db->get_where('tbl_commune_withdrawal_approval',array('loan_id' => $loan_id,'review' => NULL,'status' => 1))->row_array();
				...
				if($this->Common_model->updateRow('tbl_commune_withdrawal_approval','id', $next_userworkflow['id'], $newstatus))
				{
					$flag1 = 1;
				}
				...
				$notify_data =  array(
						...
						'sender_id' => $user_id,
						'receiver_roleid' => $next_userworkflow['assigned_roles'],
						'message' => 'Decision - '.$role_details['name']." ".trim($decision).' this loan',
						'type' => "decision",
			...
			$comnt = 'Withdraw Decision - '.$role_details['name']." ".trim($this->input->post('decision')).' this loan, '.$commentstatus;
	...
	public function confirm_withdraw_disbursement()
	{
		sleep(3);
		...
		$loan_id=$this->input->post('cl_aid');	
		$withdraw_id =  $this->input->post('record_id');
		$status =  array('status' => "Disbursement");

		if($this->Common_model->updateRow('tbl_commune_withdraw', 'id',$withdraw_id, $status))
		{
			$decision_data =  array(
									'loan_id' => $loan_id,
									'user_id' => $user_id,
			...
			// Current Person of Workflow 
			$current_userworkflow =  $this->db->get_where('tbl_commune_withdrawal_approval',array('loan_id' => $loan_id,'withdraw_id' => $withdraw_id,'assigned_roles' => $role_id,'status' => '1'))->row_array();

			//print_r($current_userworkflow); die;

			if($this->Common_model->updateRow('tbl_commune_withdrawal_approval','id', $current_userworkflow['id'], $decision_data)){
				$flag = 1;

				$decision_userids =  $this->db->get_where('tbl_commune_withdrawal_approval',array('loan_id','withdraw_id' => $withdraw_id,'id !=' => $current_userworkflow['id'],'status' => '1'))->result_array();
				...
				foreach($decision_userids as $row){
					...
					$notify_arr =  array(
						...
						'sender_id' => $user_id,
						'receiver_roleid' => $row['assigned_roles'],
						'message' => 'Withdraw Decision - '.$role_details['name'].' Final Disbursement this loan',
						'type' => "decision",
					);
				...
				// update status of previous workflow
				$new_status  =  array('status' => '0');
				$this->Common_model->updateRow('tbl_commune_withdrawal_approval','withdraw_id',$withdraw_id,$new_status);
