...
class Business_Product extends Admin_Controller
{
	...
	public function manage_business_loan_data($loan_id){
	...
	public function view_uploaded_documents($param,$param1){
		...
		if($param == "gage_espece")
		{
			...
			$uploaded_url  =  "assets/business_products/gage_espece_loans/".$folder_name."/";
	...
	public function comment_manager($type,$loan_id){
		...
		{
			if($decision =="Avis Favorable" || $decision =="Avis d̩favorable" || $decision == "Demande de retraitement")
			{
				$decision_data =  array(
									'loan_id' => $loan_id,
									'user_id' => $user_id,
			...
			if( $decision != "Avis d̩favorable" && $decision != "Demande de retraitement")
			{
	...
	public function confirm_disbursement($type){
		...
		if($this->Common_model->updateRow('tbl_gage_espece_applicationloan', 'loan_id',$loan_id, $status))
		{
			$decision_data =  array(
									'loan_id' => $loan_id,
									'user_id' => $user_id,
