...
// Usage : This controller maintains credit confort products functionalities in the application

defined('BASEPATH') OR exit('No direct script access aloowed');

class Credit_Confort extends Admin_Controller 
{
	public function __construct()
	{
		parent::__construct();
		$this->not_logged_in();
		$this->data['title'] = 'GABON | PP Credit Confort';	
		$this->data['page'] = 'PP_Credit_Confort';
	...
	public function create_file()
	{
		...
		$content = "Numéro Prêt Digital Crédit;Référence de l'Emprunteur;Catégorie de Prêts;Nominal du Prêt;Durée;Date d'Accord;Date de Réalisation;Date Première Échéance;Date Naissance Emprunteur;Numéro Compte Emprunteur;Numéro Compte Destinataire;Taux Intérêt Fixe;Périodicité des Amortissements;Barême de l'Assurance;Code Décisionnaire;Montant Garanti;Référence de la Garantie";
	    $fp = fopen($_SERVER['DOCUMENT_ROOT'] . "/development1/dcp_gabon/myText.txt","w") or die('Cannot open file');
	    fwrite($fp,$content);
		fclose($fp);

		// echo $strContent = file_get_contents("http://myprojectdemonstration.com/development1/dcp_gabon/myText.txt");
		// $strContent = mb_convert_encoding($strContent, "UTF-8", "CP936");


		// $filename = "newtext.txt";

		// $fp1 = fopen($_SERVER['DOCUMENT_ROOT'] . "/development1/dcp_gabon/".$filename, "w"); 
	 //  	fwrite($fp1, $strContent);
	 //  	fclose($fp1);
	...
	public function index($param=''){
		...
		$this->data['loan_details'] =  $this->Credit_Confort_Model->get_loan_details_branchwise();
		...
		$this->render_template2('customer_product/credit_confort/index', $this->data);
	...
	public function assign_status()
	{
	    ...
	    if($assign_status == "overide"){
    	    ...
    	    echo json_encode($output);
	         ...
	         $c_where  = "loan_id = '$loan_id' AND loan_type='credit_confort' AND assign_user_id = '$user_id' AND tbl_user.is_admin='$role_id' AND currently_active = '1' AND assign_status = 'assign_to_myself'";
        	 $check_selfuser = $this->db->query("SELECT tbl_loan_assignee.*,tbl_user.id as user_id,tbl_user.user_name,tbl_user.is_admin,tbl_user.type FROM `tbl_loan_assignee` inner join tbl_user on tbl_loan_assignee.assign_user_id = tbl_user.id WHERE $c_where ORDER BY id DESC LIMIT 1")->result_array();
        	   ...
        	   {
        	       $o_where  = "loan_id = '$loan_id' AND loan_type='credit_confort' AND tbl_user.is_admin='$role_id' AND currently_active = '1'";
	  ...
	  public function overideloan_to_user(){
	    ...
	    $u_where = "loan_id = '".$result['loan_id']."' AND loan_type = 'credit_confort'";
	...
	public function assign_attribuer(){
	    $loan_id = $this->input->post('loan_id');
	    $result  = $this->db->query("SELECT * FROM tbl_all_applications WHERE loan_id = '$loan_id' AND status = '1' AND loan_type='credit_confort' AND assigned_roles='2' ORDER BY app_id DESC LIMIT 1")->row_array();
	    
	    $user_id  = $this->session->userdata('id');
	    $user_data = $this->db->get_where('tbl_user',array('is_admin' =>$user_id))->row_array();
	    
	    $current_assign = $this->db->get_where('tbl_credit_confort_applicationloan',array('loan_id'=>$loan_id))->row_array();
	...
	public function update_assignee(){
	  ...
	  // print_r($result);
	   ...
	   if($this->Common_model->insertRow('tbl_loan_assignee',$result))
	   {
	       ...
	       $this->db->update('tbl_credit_confort_applicationloan');
	...
	// End change assignment on user roles
	...
	public function select($loan){
		...
		// $this->data['individual_details'] = $this->Customer_Model->get_customer_branchwise('individual',$searchUser);
		...
		$this->render_template2('customer_product/credit_confort/select_customer', $this->data);
	}
	...
	public function save_customer_loan()
	{
		...
		$res =  $this->Credit_Confort_Model->save_loan_application($customer_id,$sub_product,$customer_type,$loan_Id);
	}
	...
	public function customer_details($loan_id,$action){
		...
		$this->data['record'] = $this->Common_model->get_admin_details();
		$this->data['dashboard_url'] =  'Dashboard';
		$this->data['page_title'] =  'PP CREDIT CONFORT';
		...
		if($action == "edit"){
		   ...
		   $check_assigned_role = $this->Common_model->check_assignedrole_workflow('credit_confort',$this->data['record'][0]->is_admin,$loan_id);
		...
		$this->data['loan_count'] = $this->Credit_Confort_Model->get_number_of_loans($this->data['loan_details']['customer_id']);
	    $d_where  =  array('loan_id' => $loan_id,'loan_type' => "credit_confort",'assigned_roles' => $this->data['record'][0]->is_admin,'status' => "1");
		$this->data['current_user_decision'] = $this->Common_model->getRecord('tbl_all_applications','',$d_where);
		$this->data['loan_overallstatus'] = $this->Credit_Confort_Model->get_overall_loan_status($loan_id);
		$this->data['decision_rec'] =  $this->Credit_Confort_Model->get_approval_decision_details($loan_id,"credit_confort");
		
		$amort_where  = array('applicationform_id' => $this->data['loan_details']['loan_id_temp'],'loan_type' => '3');
		$amortization =  $this->Common_model->getRecord('tbl_consumer_amortization','',$amort_where);
		//print_r($amortization); die;
	
		$this->data['loan_databinding'] = json_decode($amortization['databinding']);
	//	print_r($this->data['loan_databinding']); die;
	    ...
	    $this->data['risk_analysis'] =  $this->Credit_Confort_Model->get_risk_analysis_details($loan_id);
		$this->data['tracking_timeline'] =  $this->Credit_Confort_Model->get_tracking_details($loan_id);
		$this->data['history_interaction'] =  $this->Credit_Confort_Model->get_interaction_history($loan_id);



		// $this->data['collateral']=$this->Credit_Confort_Model->get_collateral($loan_id);
		// $this->data['collateral_vehicule']=$this->Credit_Confort_Model->get_collateral_vehicule($loan_id);
		// $this->data['collateral_deposit']=$this->Credit_Confort_Model->get_collateral_deposit($loan_id);
		// $this->data['collateral_maison']=$this->Credit_Confort_Model->get_collateral_maison($loan_id);
		// $this->data['collateral_excemption']=$this->Credit_Confort_Model->get_collateral_excemption($loan_id);


		// $this->data['riskcurrentmonthlyvrefit']=$this->Credit_Confort_Model->get_current_monthly_credit($loan_id);
		// $this->data['riskpaymentnewloan']=$this->Credit_Confort_Model->get_monthly_payment_new_loan($loan_id);
		// $this->data['riskfsituation']=$this->Credit_Confort_Model->get_financial_situation($loan_id);
		...
		$this->data['documents'] =  $this->Credit_Confort_Model->get_saved_documents($loan_id);
		$this->data['secteurs']=$this->Secteur_Model->get_all_secteurs();
		$this->data['employers'] =  $this->Employer_Model->get_all_employers();
		$this->data['cat_emp']=$this->EmployeeCategory_Model->get_all_categories();
		$this->data['contract_type']=$this->PP_Consumer_Loans_Model->get_type_of_contract();
		...
		// {
			$this->render_template2('customer_product/credit_confort/customer_details', $this->data);
		//}
		// else
		// {
	...
	public function edit_loan(){  
		...
		$check_amort =$this->PP_Consumer_Loans_Model->get_Post_temp_applicationform($id);
            if($loan_amount<2000000){
                echo "Le montant doit être supérieur à 2 000 000";
                die;
            }
            if($loan_amount>5000000){
                echo "Le montant doit être inférieur à 5 000 000";
                die;
            }
		...
		{ 
		   ...
		   $risk_analysis = $this->db->get_where('tbl_credit_confort_risk_analysis',array('loan_id'=>$loan_id))->row_array();
		   ...
		   if($total_rem != 0 && $total_salary != 0)
		    ...
		    $this->Common_model->updateRow('tbl_credit_confort_risk_analysis','loan_id',$loan_id,$updata);
				...
				if ($this->form_validation->run() == FALSE)
				{
					...
					if($getid){			   
						...
						$filename = $this->upload->data();
							...
							$details="Collateral Véhicule ".$filesCount." téléchargement de fichiers.";
							$history_arr=array(
								"cl_aid" =>$_POST['loan_id'],
								"admin_id" =>$this->session->userdata('id'),
							...
							$output=array('success' => $filesCount." Fichiers téléchargés avec succès.");
							}
						}
					}
			   }			   
						...
						if($getid){			   
						   for($count =0; $count<$filesCount; $count++)
						   {
							...
							if(!$this->upload->do_upload('file'))
							{
								...
								$rsid=$this->Common_model->insertRow('collateral_files',$filedata);
								$details="Collateral Déposit ".$filesCount." téléchargement de fichiers.";
								$history_arr=array(
									"cl_aid" =>$_POST['loan_id'],
									"admin_id" =>$this->session->userdata('id'),
					...
					{			   
						for($count =0; $count<$filesCount; $count++)
						{
							...
							if(!$this->upload->do_upload('file'))
							{
								...
								$rsid=$this->Common_model->insertRow('collateral_files',$filedata);
								$details="Collateral Maison ".$filesCount." téléchargement de fichiers.";
								$history_arr=array(
									"cl_aid" =>$_POST['loan_id'],
									"admin_id" =>$this->session->userdata('id'),
									"customar_id" =>$this->input->post('customar_id'),
			...
			if($getid)
			{
			   for($count =0; $count<$filesCount; $count++)
			   {
				...
				if(!$this->upload->do_upload('file'))
				{
					...
					$rsid=$this->Common_model->insertRow('collateral_files',$filedata);
					$details="Collateral Excemption ".$filesCount." téléchargement de fichiers.";
					$history_arr=array(
						"cl_aid" =>$_POST['loan_id'],
						"admin_id" =>$this->session->userdata('id'),
						"customar_id" =>$this->input->post('customar_id'),
...
//Document System forms---- COONVENTION DE CREDIT
	...
	{
		 error_reporting(0);
		$this->data['appformD']=$this->Credit_Confort_Model->get_single_loan_details($id);
		//print_r($this->data['appformD']); die;
// 		$customerID=$this->data['appformD'][0]['customar_id'];
// 		$this->data['loantax']=$this->PP_Consumer_Loans_Model->get_taxType();
// 		$this->data['loandetails']=$this->PP_Consumer_Loans_Model->get_new_application_form();
// 		$this->data['tracking_timeline']=$this->PP_Consumer_Loans_Model->get_tracking_timeline($id);
...
// 		$this->data['applicableloanratio']=$this->PP_Consumer_Loans_Model->get_applicableloan_ratio();
		...
		public function generate_memo_of_setting_up_Credit_Scolaire($id)
	{
		 error_reporting(0);
		$this->data['appformD']=$this->Credit_Confort_Model->get_single_loan_details($id);
		$this->data['loan_type']='credit_confort';
...
// 		$this->data['applicableloanratio']=$this->PP_Consumer_Loans_Model->get_applicableloan_ratio();
		...
		public function generate_formulaire_de_demande_credit_scolaire($id)
	{
		...
		$this->data['appformD']=$this->Credit_Confort_Model->get_single_loan_details($id);
		$customerID=$this->data['appformD'][0]['customar_id'];
		$this->data['risk_analysis'] =  $this->Credit_Confort_Model->get_risk_analysis_details($id);
		// $this->data['loantax']=$this->PP_Consumer_Loans_Model->get_taxType();
		// $this->data['loandetails']=$this->PP_Consumer_Loans_Model->get_new_application_form();
		// $this->data['tracking_timeline']=$this->PP_Consumer_Loans_Model->get_tracking_timeline($id);
		// $this->data['fees']=$this->PP_Consumer_Loans_Model->get_All_fees();
		...
		$this->data['riskcurrentmonthlyvrefit']=$this->Credit_Confort_Model->get_current_monthly_credit($id);
		// $this->data['riskpaymentnewloan']=$this->PP_Consumer_Loans_Model->get_monthly_payment_new_loan($id);
		// $this->data['riskfsituation']=$this->PP_Consumer_Loans_Model->get_financial_situation($id);
		// $this->data['applicableloanratio']=$this->PP_Consumer_Loans_Model->get_applicableloan_ratio();
		return  $this->load->view('superadmin/decision_old',$this->data,true);
	}
...
// 		die;
		...
		$check_loan =  $this->db->get_where('tbl_credit_confort_risk_analysis',array('loan_id' => $loan_id ))->row_array();

	   	if(empty($check_loan)){

	   		$this->Common_model->insertRow('tbl_credit_confort_risk_analysis',$data);
	   	}
	    else
	    {
	   	  	$this->Common_model->updateRow('tbl_credit_confort_risk_analysis','loan_id',$loan_id,$data);
	    }
	}
	...
	public function riskAnalysis_coefficient_remboursement(){
		...
		$data['solde_encour_credit_bail'] = $this->input->post('solde_encour_credit_bail');
		...
		if(empty($check_loan)){
			...
			$work_where =  array('type' => "credit_confort",'deleted' => "0");
			...
			if($workflow_id)
			{
				// delete previous data which is inserted when loan is created
				 ...
				 $this->db->where('loan_type',"credit_confort");
	...
	public function archive_loan(){
		...
		$this->data['loan_details'] =  $this->Credit_Confort_Model->get_archive_loans(); 
		$this->data['controller'] = $this; 

		$this->render_template2('customer_product/credit_confort/archive_loan', $this->data);
	}
	
	public function contractDePret()
	{
		error_reporting(0);
		$segmentid=$this->uri->segment(3);
		$getHtmlagreement=$this->generate_contractDePret($segmentid);
		echo $getHtmlagreement;
		$filename=$segmentid.'-'.time();
	}


		public function generate_contractDePret($id)
	{
		error_reporting(0);
		$this->data['appformD']=$this->Credit_Confort_Model->get_single_loan_details($id);
		$customerID=$this->data['appformD'][0]['customar_id'];
		...
		// $this->data['riskfsituation']=$this->PP_Consumer_Loans_Model->get_financial_situation($id);
		// $this->data['applicableloanratio']=$this->PP_Consumer_Loans_Model->get_applicableloan_ratio();
		return  $this->load->view('superadmin/contractDePret',$this->data,true);
	}
	
		public function letterDeDecision()
	{
		error_reporting(0);
		$segmentid=$this->uri->segment(3);
		$getHtmlagreement=$this->generate_letterDeDecision($segmentid);
		echo $getHtmlagreement;
		$filename=$segmentid.'-'.time();
	}


		public function generate_letterDeDecision($id)
	{
		error_reporting(0);
		$this->data['appformD']=$this->Credit_Confort_Model->get_single_loan_details($id);
		$customerID=$this->data['appformD'][0]['customar_id'];
		...
		// $this->data['riskfsituation']=$this->PP_Consumer_Loans_Model->get_financial_situation($id);
		// $this->data['applicableloanratio']=$this->PP_Consumer_Loans_Model->get_applicableloan_ratio();
		return  $this->load->view('superadmin/letterDeDecision',$this->data,true);
	}
