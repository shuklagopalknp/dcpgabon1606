		...
		if($_FILES["files"]["name"] != '')
		{
		   ...
		   for($count = 0; $count<count($_FILES["files"]["name"]); $count++)
		   {
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$output=array('success' => $filesCount." Fichier téléchargé avec succès.");
			}
		   }
		   if(!empty($output['success'])){
	  			$details="Account Manager ".$filesCount." Fichier téléchargé avec succès";
				$history_arr=array(
					"loan_id" =>$param1,
		...
		if($_FILES["files"]["name"] != '')
		{
		   ...
		   for($count = 0; $count<count($_FILES["files"]["name"]); $count++)
		   {
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$rsid=$this->Common_model->insertRow('tbl_business_documents',$data);
				$output=array('success' => $filesCount."Les fichiers ont été téléchargés avec succès.");
			}
		   }
		   if(!empty($output['success'])){
	  			$details="Account Manager ".$filesCount."Les fichiers ont été téléchargés avec succès.";
				$history_arr=array(
					"loan_id" =>$param1,
					"user_id" =>$this->session->userdata('id'),
...
public function uploadfile_risk_analysis($param,$param1)
	...
	public function comment_manager($type,$loan_id){
		...
		{
			if($decision =="Avis Favorable" || $decision =="Avis défavorable" || $decision == "Demande de retraitement" || $decision == "Annuler" || $decision == "Abandonner")
			{
				$decision_data =  array(
									'loan_id' => $loan_id,
									'user_id' => $user_id,
			...
			if(  $decision != "Demande de retraitement" && $decision != "Annuler" && $decision !="Abandonner")
			{
                ...
                if($next_userworkflow['assigned_roles'] == "12" && $decision =="Avis défavorable")
				{
				    
			        $cancel_status = "Annulé";
		  ...
		  //      echo $this->db->last_query();
			...
			$comnt = 'Decision - '.$role_details['name']." ".trim($this->input->post('decision')).' pour ce prêt, '.$this->input->post('commentstatus');
			...
			if($this->Common_model->updateRow('tbl_all_applications','app_id', $current_userworkflow['app_id'], $decision_data)){
			 ...
			 //	Loan Details
				...
				$notify_data1 =  array(
										...
										'sender_id' => $user_id,
										'receiver_id' => '1',
										'message' => 'Decision - '.$role_details['name'].' Décaissement du Prêt avec Succès',
										'type' => "decision",
				...
				$comnt = 'Decision - '.$role_details['name']." ".'Décaissement du Prêt avec Succès';
