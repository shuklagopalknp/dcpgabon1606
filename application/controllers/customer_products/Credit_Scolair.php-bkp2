...
class Credit_Scolair extends Admin_Controller 
{
    ...
    public function assignloan_to_user($param){
	    ...
	    if($this->Common_model->insertRow('tbl_loan_assignee',$result))
        {
			...
			$this->Common_model->insertRow('tbl_business_tracking_timeline',$track_data);
    	       ...
    	       redirect('PP_CONGES_A_LA_CARTE/customer_details/'.$result['loan_id'].'/edit');
        }
	...
	//	$this->data['business_details'] = $this->Customer_Model->get_customer_branchwise('business');
		...
		$array_where1 = array('g.customer_id' => $customer_id, 'g.status' => NULL);

		$this->db->where($array_where1);
		...
		$array_where2 = array('g.customer_id' => $customer_id);

		$this->db->where($array_where2);
	
    	$result2 =  $this->db->get()->result_array();
    	
    	$trycount=count($result2);
    // 	 print_r($result2);
    //     print_r(count($result2));
    // 	 die;
    	if($trycount>=2) {
    	    $response['success']=0;
    	    $response['arrayLoan']= $result2;
            }elseif(empty($result)){
		     $response['success']=1;
...
// 		die;
	   ...
	   $loan_temp_id = $loan_details['loan_id_temp'];
		...
		$this->Common_model->updateRow('tbl_temp_consumer_applicationform','aid',$loan_temp_id,$updatedata);
   		...
   		$amortrecord = $this->db->get_where('tbl_temp_consumer_applicationform',array('aid' => $loan_temp_id ))->result_array();
    	...
    	$return_arr=array();
		   ...
		   $updata['total_engagement']  = $total_amount;
