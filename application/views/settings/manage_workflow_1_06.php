						...
						<div class="main-box-body clearfix">
							...
							<form id="tabForm" action="#"  method="post" class="form-horizontal" role="form" >
								...
								<div class="form-group">
									...
									<div class="col-md-3">
										<select name="product_type" class="form-control">
											...
											<option value="credit_conso" <?php if($details['type'] == "credit_conso") echo 'selected';?> >PP Credit Conso</option>
											<option value="caution_scolaire" <?php if($details['type'] == "caution_scolaire") echo 'selected';?>>PP Caution Scolaire</option>
								...
								<div class="form-group">
									...
									<div class="col-md-3">
										Min Amount :<input type="text" class="form-control" name="min_amount" value="<?php echo $details['min_amount'] ?? '';?>">
									</div>
									<div class="col-md-3">
										Max Amount :<input type="text" class="form-control" name="max_amount" value="<?php echo $details['max_amount'] ?? '';?>">
									</div>
								</div>
								...
								<tr <?php if(!empty($role_status) && $role_status[$role_key] == "1"){?> class="checked_role"  <?php } ?>>
									...
									<td>
										<label class="switch">
										  <input type="checkbox" id="s_<?php echo $count;?>" class="status_c" <?php if(!empty($role_status) && $role_status[$role_key] == "1") echo 'checked';?>>
										  <span class="slider round" ></span>
										</label>
										<input type="hidden" class="input_status" name="status[]" value="<?php echo $role_status[$role_key] ?? '0'; ?>">

										
									</td>

									<td>
										<span class="show_c badge badge-info" id="t_<?php echo $count;?>"><?php if(!empty($role_order) && $role_order[$role_key]) echo $role_order[$role_key];?></span>
										<input type="hidden" class="input_order" id="io_<?php echo $count ;?>" name="order[]" value="<?php echo $role_order[$role_key] ?? '0';?>">
									</td>
								</tr>	
	...
	$(".status_c").click(function(){
		...
		if($(this).is(":checked"))
		{
			...
			if($("#t_"+elemid['1']).text() <= $('.checked_role').length)
			{
				...
				{
					//alert(i);
					if($("#io_"+i).val() != "0" && $("#io_"+i).val() > $("#t_"+elemid['1']).text())
					{
						$("#t_"+i).text(parseInt($("#t_"+i).text())-1);
						$("#io_"+i).val(parseInt($("#io_"+i).val())-1);
					}
