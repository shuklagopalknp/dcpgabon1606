            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <div class="main-box-body clearfix">
                                        ...
                                        <div class="col-md-12">
                                            <div class="table-responsive">
                                                <table id="table-example1" class="table table-hover">
                                                    ...
                                                    <tbody id="callsearch">
                                                        <?php foreach ($customerdetails as $key) {
                                                        //echo "<pre>", print_r($key), "</pre>";
                                                         ?>
                                                           ...
                                                           <td>
                                                            <a href="javascript:void(0)" class="btn btn-primary" onclick="return theFunction(<?php echo $key['cid'];?>);"/><img src="<?php echo  base_url('assets/img/select2-spinner.gif');?>" class="pull-right" id="ajax-loder<?php echo $key['cid'];?>" style="position:relative;top: 2px;left: -2px; display: none;" />
                                                                SELECT
                                                            </a> 
                                                            </td>
                                                        </tr>
                        ...
                        <textarea cols="1" rows="8" class="form-control jsondata" style="display: none;"></textarea>
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    $("#searchString").on("keypress keyup blur",function (event) {
       $(this).val($(this).val().replace(/[^\d].+/, ""));
    ...
    function search(force) {   
        ...
        $.ajax({
            type:'POST',
            url:'<?php echo base_url("Decovert_Loans/SearchDecovertAccount");?>',
    ...
    function theFunction (cid) {
        if(confirm('Continue?') ) {
            var segment ="<?php echo $this->uri->segment(3);?>";
             $.ajax({
                type:'POST',
                url:'<?php echo base_url("Decovert_Loans/is_ajax_request");?>',
                ...
                success:function(resp){ 
                    console.log(resp);
                    $('.jsondata').val($.trim(resp));
                    setTimeout(function() {
                      $("#callsearch").css("opacity","");
                      window.location.href = "<?php echo base_url('Decovert_Loans/customer_details/');?>"+$.trim(resp);
                    }, 1000);
                }
              });
        }
...
<script type="text/javascript">
        ...
        submitHandler: function(form) {         
            ...
            $.ajax({
                type:'POST',
                url:'<?php echo base_url("Decovert_Loans/add_newcustomer");?>',
