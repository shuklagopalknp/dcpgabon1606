		...
		<div class="col-lg-12">
			...
			<div id="content-header" class="clearfix">
				<div class="pull-left">
					<ol class="breadcrumb">
						...
						<li class="active"><span>Fête à la Carte</span></li>
			...
			<div class="main-box-body clearfix">
				...
				<a href="javascript:void(0);" class="btn btn-primary  openBtn"  data-toggle="modal" data-target="#myModal">
					<i class="fa fa-plus-circle fa-lg"></i> Nouvelle demande de Prêt FAC
				</a>
				<?php } ?>
				<a href="<?php echo base_url('PP_FETES_A_LA_CARTE/archive_loan')?>" class="btn btn-primary pull-right">
											...
											<tr id="rowid<?php echo $ld['loan_id'];?>">
											  ...
											  //  print_r($app_status);
    											    			...
    											    			if($app_status['approval_status'] == "Process")
        											    		{
        											    		    if($this->session->userdata('role') == "12" && $last_data['workflow_order'] == $app_status['workflow_order'] )
        											    		    {
        											    		     if (in_array('4_6', $this->session->userdata('portal_permission'))){
                                                                	          ...
                                                                	          $disabled = $this->Common_model->get_current_status($ld['loan_id'],'credit_conso',$this->session->userdata('role'));
											 	...
											 	<td class="sorting_1">
													...
													<a href="<?php echo base_url('PP_FETES_A_LA_CARTE/customer_details/').$ld['loan_id'];?>/edit" class="table-link">
														<span class="fa-stack">
															<i class="fa fa-square fa-stack-2x"></i>
													...
													if($this->Common_model->check_assignedrole_workflow('credit_conso',$this->session->userdata('role'),$ld['loan_id'])){
													?>
													<div class="dropdown">
													...
													<?php if($ld['final_status'] == "Disbursement"){
                                                        ...
                                                        //     }
													?>
														<a href="<?php echo base_url('Customer_Product/archive_loan/credit_conso/').$ld['loan_id'];?>" class="table-link">
	  	...
	  	<div class="modal-dialog modal-full-height modal-left modal-notify modal-info">	
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title heading lead">Nouvelle demande de Prêt FAC</h4>
				</div>
					...
					<div class="modal-body">   
						...
						<div class="form-group">
							...
							<select class="form-control" id="loan_object_drop" onchange='add_object(this.value);' required>
								...
								<option value="fetes a la carte" selected >Fetes a la carte</option>
								<option value="other">Other</option>
							</select>
								<input type="text" class="form-control" id="loan_object"  name="loan_object"  value="fetes a la carte" style='display:none;' >
...
<div class="modal fade" id="show_popup" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="<?php echo base_url()?>PP_FETES_A_LA_CARTE/update_assignee" method="POST">
...
<div class="modal fade" id="overide_popup" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="<?php echo base_url()?>PP_FETES_A_LA_CARTE/overideloan_to_user" method="POST">
            <div class="modal-body">
...
function set_status(val1,val2){
    ...
    $.ajax({
        url:"<?php echo base_url()?>PP_FETES_A_LA_CARTE/assign_status",
        type:"POST",
        ...
        success:function(response){
            ...
            {  $("#other_popup").find('.modal-body').text("Cette demande vous est déjà assignée"); //You are already assigned in this loan
               $("#other_popup").modal('show');
                ...
                $("#cnfrm_delete").find("a.yes-btn").attr("href","<?php echo base_url()?>PP_FETES_A_LA_CARTE/assignloan_to_user/"+val1);
...
function set_attribuer(val){
    $.ajax({
       url:"<?php echo base_url()?>PP_FETES_A_LA_CARTE/assign_attribuer",
       type:"POST",
       data:{'loan_id':val},
       success:function(response){
            var result  =  JSON.parse(response);
				...
				success: function(response) {
					...
					if(resp.error){
						$('.error-msg').html('<div class="alert alert-danger"><i class="fa fa-times-circle fa-fw fa-lg"></i><strong>Error!</strong>'+$.trim(resp.error)+'.</div>');               			           	
	                     ...
	                     setTimeout(function() {						
							window.location.href ="<?php echo base_url('PP_Consumer_Loans/amortization_loan/');?>"+$.trim(resp.success)+'/1'+"";
						}, 500);						
                 	} 
				}
			});
		...
		function loanDisbursement(event)
	{
	   ...
	   // alert (st);
		if (confirm('Voulez-vous vraiment le décaissement du prêt?')) {			
			$.ajax({
		            url: '<?php echo base_url("Customer_Product/confirm_disbursement/credit_conso");?>',
		            type: "POST",
		            data: {
		                'cl_aid':event
		            },
