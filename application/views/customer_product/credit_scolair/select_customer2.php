							...
							<div class="main-box-body clearfix">
								...
								<div id="Customer" class="tabcontent">
									...
									<div class="row">
											<div class="col-md-12">
												<div class="col-md-4">
													<form action ="<?php echo base_url("PP_CONGES_A_LA_CARTE/select/").$loan.'/'.$department?>" method="POST">
													    <div class="searchFor">
										...
										<tbody id="callsearch">
											...
											foreach ($individual_details as $row) {
											 ...
											 ?>
												...
												<td>
													<a href="javascript:void(0)" class="btn btn-primary" onclick="return theFunction(<?php echo $row['customer_id'];?>,'1',<?php echo $loan;?>,'<?php echo $department?>');"/><img src="<?php echo  base_url('assets/img/select2-spinner.gif');?>" class="pull-left" id="ajax-loder<?php echo $row['customer_id'];?>" style="position:relative;top: 2px;left: -2px; display: none;" /> SELECT
													</a>
											</td>
											</tr>
											<?php } ?>
...
function search(force) {
	...
	function theFunction (customerid,type,loan,dept) {
        ...
        if(confirm('Continuez?') ) {
            ...
            $.ajax({
				type:'POST',
				url:'<?php echo base_url("PP_CONGES_A_LA_CARTE/check_single_loan");?>',
				...
				success:function(resp){ 
				    ...
				    if(result.success=='0'){
				        ...
				        $.ajax({
				type:'POST',
				url:'<?php echo base_url("PP_CONGES_A_LA_CARTE/save_customer_loan");?>',
				data: {customer_id:customerid,sub_product:sub_product,type:type,loan_id:loan,department:dept},
				beforeSend: function(){        	
		        	$('#callsearch').css("opacity","0.5");
		        	$('#ajax-loder'+customerid).css("display","inline");
				...
				success:function(resp){ 
			     ...
			     //	return false;
					...
					$('#callsearch').css("opacity","");											
						if(resp){
						    if(user_role == "2"){
					        	window.location.href = "<?php echo base_url('PP_CONGES_A_LA_CARTE/customer_details/');?>"+$.trim(resp)+"/edit";
						    }
						    else{
					        	window.location.href = "<?php echo base_url('PP_CONGES_A_LA_CARTE/customer_details/');?>"+$.trim(resp)+"/view";
						    }
