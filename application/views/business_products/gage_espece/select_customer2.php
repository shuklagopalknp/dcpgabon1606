										...
										<tbody id="callsearch">
											...
											foreach ($business_details as $row1) {
											 ...
											 ?>
												...
												<td><a href="javascript:void(0);" onClick="editbusinesscustomermodal(<?php echo $row1['customer_id'];?>)"><?php echo $row1['company_name'];?></a></td>
...
function search(force) {
	...
	function theFunction (customerid,type) {
        ...
        if(confirm('Continue?') ) {
        	...
        	$.ajax({
				type:'POST',
				url:'<?php echo base_url("Gage_Espece/save_customer_loan");?>',
				...
				success:function(resp){ 
					...
					$('#callsearch').css("opacity","");											
						...
						if(resp == "tab_error"){
							...
							window.location.href = "<?php echo base_url('Gage_Espece/customer_details/');?>"+resp;
						}
					}, 1500);
				}      
		     });
