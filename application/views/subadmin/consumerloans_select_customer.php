...
<div id="EditCustomerModal" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static">
  	<div class="modal-dialog modal-full-height modal-left modal-notify modal-info">	
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h4 class="modal-title heading lead">Add Customar</h4>
			</div>
...
<script type="text/javascript">
	function editcustomermodal(id)	{
	 ...
	 $.ajax({
	 	type: "POST",
	 	url:'<?php echo base_url("PP_Consumer_Loans/editcustomer");?>',
	 	data:{id:id},
	 	beforeSend: function(){
...
<script type="text/javascript">
		...
		submitHandler: function(form) {			
			...
			$.ajax({
				type:'POST',
				url:'<?php echo base_url("PP_Consumer_Approval_Bank/add_newcustomer");?>',
...
function search(force) {
	...
	function theFunction (customerid) {
        if(confirm('Continue?') ) {
        	...
        	$.ajax({
				type:'POST',
				url:'<?php echo base_url("PP_Consumer_Approval_Bank/is_ajax_request");?>',
				...
				success:function(resp){ 
					...
					$('#callsearch').css("opacity","");											
						if(resp !=0){
							window.location.href = "<?php echo base_url('PP_Consumer_Approval_Bank/consumerloan/');?>";
						}
					}, 1500);
				}      
		     });
