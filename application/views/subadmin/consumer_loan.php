									...
									<table id="table-example" class="table table-hover">
										...
										<tbody id="getrecord">
											...
											<tr id="rowid<?php echo $key['cl_aid'];?>">
												...
												<td>
													<?php echo $key['clientfirstname'] ?: '';?><?php echo $key['clientmiddlename'] ?: ' ';?><?php echo $key['clientlastname'] ?: '';?>
													<?php if(!$this->PP_Consumer_Loans_Model->get_decision_user_status($key['cl_aid']))
													{ ?>
														<br>
														<a href="<?php echo base_url("PP_Consumer_Approval_Bank/select_customer/".$key['cl_aid']); ?>">Change Customer</a>
														<?php
													}?>		
												</td>
												...
												<td>F CFA <?php echo number_format($appdata[0]->loan_amt,0,',',' ');?></td>
												...
												<td class="sorting_1">
													<a href="<?php echo base_url('PP_Consumer_Approval_Bank/customer_details/').$key['cl_aid'];?>" class="table-link">
														<span class="fa-stack">
...
<script type="text/javascript">
	$(document).ready(function() {
	  	$('#search_status').change(function() {
		    var filter = $(this).val();
		    var branch_id= "<?php echo number_format($loandetails[0]['branch_id'],0,',','');?>";
		    //alert(filter);
		    //alert(branch_id);
		    $.ajax({
		        type:'POST',
		        url:'<?php echo base_url("PP_Consumer_Approval_Bank/searchloanstatus");?>',
...
<script>
	...
	function loanDisbursement(event)
	{
		if (confirm('Are you sure you want to loan disbursement?')) {			
			$.ajax({
		            url: '<?php echo base_url("PP_Consumer_Approval_Bank/confirm_disbursement");?>',
		            type: "POST",
		            data: {
		                'cl_aid':event
		            },
