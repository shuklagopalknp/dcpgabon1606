...
class Dynamic_menu {
    ...
    private $class_menu        = 'class="nav nav-pills nav-stacked"';
    ...
    {
        $menu = array();
       // print_r($this->ci->session->userdata('role'));
 
     $query = $this->ci->db->query("select * from dyn_menu where role_id=".$this->ci->session->userdata('role')); 
 ...
 
              {
 ...
 
                {
                    ...
                    // CodeIgniter's anchor(uri segments, text, attributes) tag.
                        ...
                        $html_out .= "\t\t\t".'<li class=""> '.anchor($url.'" '.$this->class_parent , '<span>'.$title.'</span>','name="'.$title.'" id="'.$id.'" target="'.$target.'"');
