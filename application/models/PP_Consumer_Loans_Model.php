...
class PP_Consumer_Loans_Model extends CI_Model
{
	...
	public function get_customer_personalinformation($id)
	{
		$result  =  $this->db->get_where('tbl_customer_personalinformation',array('customar_id'=>$id))->row_array();
	...
	# Get agent cad decision form
	public function get_agentcad_decision_form($loanId) 
	{		
		$result = $this->db->select('*')->from('tbl_agent_cad_decision_form')->where('loan_id', $loanId)->get()->row_array();
	...
	public function get_temp_applicationformRecord($segment){
		...
		$query=$this->db->select('application_no,ip_address,loan_type,loan_amt,loan_interest,loan_term,loan_schedule,loan_fee,loan_tax,loan_commission,created,loan_guarantee')
			->from('temp_consumer_applicationform')
			->where('status',1)
			->where('aid',$segment)	
			->limit(1)		
	...
	public function get_decision_user_status($id){		
		$query=$this->db->select('b.*,u.*, r.field_data')
			->from('branmanager_approbation as b')	
			->join('user u','b.manager_id = u.id','inner')	
			->join('role r','u.is_admin = r.id','inner')		
