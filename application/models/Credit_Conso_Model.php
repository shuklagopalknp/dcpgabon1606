...
class Credit_Conso_Model extends CI_Model
{
	...
	public function save_loan_application($customer_id,$sub_product,$customer_type,$loan_id){
		...
		$product_tab =  $this->db->get_where('tbl_details_tabs_consumer_products',array('status' => '1','product_type' => 'credit_conso'))->result_array();
		...
		$system_docs  =  $this->db->get_where('tbl_document_system',array('document_status' => '1','product_type' => 'credit_conso','document_type' => "system_docs",'sub_product_type' => $sub_product))->result_array();
		...
		$checklist_docs  =  $this->db->get_where('tbl_document_system',array('document_status' => '1','product_type' => 'credit_conso','document_type' => "checklist_docs",'sub_product_type' => $sub_product))->result_array();
		$checklist_arr[] = '';
		$chk_status[] =  '0';
		...
		$risk_analysis_docs  =  $this->db->get_where('tbl_document_system',array('document_status' => '1','product_type' => 'credit_conso','document_type' => "risk_analysis_docs",'sub_product_type' => $sub_product))->result_array();
		...
		if($insert_id =  $this->Common_model->insertRow('tbl_credit_conso_applicationloan',$insertdata))
		{
			$doc_data  =  array(
								'loan_id'=> $insert_id,
			...
			$this->Common_model->insertRow('tbl_credit_conso_documents',$doc_data);
		    ...
		    $this->Email_Sms_Template_Model->send_mail_template2($customer_data,$customer_type,$insertdata['application_no'],"FETES A LA CARTE");

			// Send sms To customer
			 $this->Email_Sms_Template_Model->send_sms_template2($customer_data,$customer_type,$insertdata['application_no'],"FETES A LA CARTE");
	...
	public function get_saved_documents($loan_id){
		$result =  $this->db->get_where('tbl_credit_conso_documents',array('loan_id' => $loan_id))->row_array();
	...
	public function get_loan_details_branchwise()
	{
		...
		$this->db->select('g.loan_id,g.user_id created_by,g.application_no,g.sub_product,g.customer_data,g.created_at,g.modified_at,b.branch_name,b.department,g.customer_type,g.final_status,loan.loan_amt,loan.loan_interest,loan.loan_term,loan.loan_schedule,loan.loan_fee,loan.loan_tax,loan.teg_rate,loan.wear_rate,loan.created,loan.loan_guarantee,loan.annual_teg,loan.periodic_teg,loan.loan_object');
...
// 		return $result  =  $this->db->get_where('tbl_all_applications',array('loan_id' => $loan_id,'loan_type' => $type,'review' => 1))->result_array();
	...
	public function get_overall_loan_status($loan_id){
		...
		$d_where  =  array('loan_id' => $loan_id,'loan_type' => "credit_conso",'review !='=>NULL,'status' => "1");
	    ...
	    $loan = $this->db->get_where('tbl_credit_conso_applicationloan',$l_where)->row_array();
	...
	public function get_risk_analysis_details($loan_id){
		...
		$result =  $this->db->get_where('tbl_credit_conso_risk_analysis',array('loan_id' =>$loan_id))->row_array();
		return $result;
...
// 	{
// 		$result =  $this->db->get_where('tbl_credit_conso_risk_analysis_facility',array('loan_id' =>$loan_id))->row_array();
// 		return $result;
// 	}
	...
	public function get_tracking_details($loan_id)
	{
		...
		$this->db->join('tbl_credit_conso_applicationloan as loan','loan.loan_id = track.loan_id','inner');
		$this->db->join('tbl_user as user','user.id =  track.user_id','inner');
		$this->db->where('track.loan_id',$loan_id);
		$this->db->where('track.product_type','credit_conso');
	...
	public function get_interaction_history($loan_id){
		$result['email']  =  $this->db->get_where('tbl_customer_interaction_history',array('loan_id' => $loan_id,'type' => "credit_conso",'mode' =>  "email"))->result_array();

		$result['sms']  =  $this->db->get_where('tbl_customer_interaction_history',array('loan_id' => $loan_id,'type' => "credit_conso",'mode' =>  "sms"))->result_array();

		$result['interview']  =  $this->db->get_where('tbl_customer_interaction_history',array('loan_id' => $loan_id,'type' => "credit_conso",'mode' =>  "interview_telephone"))->result_array();
	...
	public function get_collateralFiles($id){
		...
		public function get_number_of_loans($id){
			...
			$num_rows = $this->db->count_all_results('tbl_credit_conso_applicationloan');
			return $num_rows;
			//echo  $this->db->last_query();
