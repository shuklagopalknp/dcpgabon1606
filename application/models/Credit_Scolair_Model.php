...
class Credit_Scolair_Model extends CI_Model
{
	...
	public function save_loan_application($customer_id,$sub_product,$customer_type,$loan_id,$department){
...
//         print_r($customer_data[0]['contract_type_id']);
// 		print_r($customer_data[0]['cat_employeurs']);
// 		die;
...
//  		die;
		...
		$product_tab =  $this->db->get_where('tbl_details_tabs_consumer_products',array('status' => '1','product_type' => 'credit_scolair'))->result_array();
		...
		$system_docs  =  $this->db->get_where('tbl_document_system',array('document_status' => '1','product_type' => 'credit_scolair','document_type' => "system_docs",'sub_product_type' => $sub_product))->result_array();
		...
		$checklist_docs  =  $this->db->get_where('tbl_document_system',array('document_status' => '1','product_type' => 'credit_scolair','document_type' => "checklist_docs",'sub_product_type' => $sub_product))->result_array();
		$checklist_arr[] = '';
		$chk_status[] =  '0';
		...
		$risk_analysis_docs  =  $this->db->get_where('tbl_document_system',array('document_status' => '1','product_type' => 'credit_scolair','document_type' => "risk_analysis_docs",'sub_product_type' => $sub_product))->result_array();
		...
		if($insert_id =  $this->Common_model->insertRow('tbl_credit_scolair_applicationloan',$insertdata))
		{
			$doc_data  =  array(
								'loan_id'=> $insert_id,
			...
			$this->Common_model->insertRow('tbl_credit_scolair_documents',$doc_data);
...
// 		    $this->Email_Sms_Template_Model->send_mail_template2($customer_data,$customer_type,$insertdata['application_no'],"CONGES A LA CARTE");

// 			// Send sms To customer
// 			 $this->Email_Sms_Template_Model->send_sms_template2($customer_data,$customer_type,$insertdata['application_no'],"CONGES A LA CARTE");
	...
	public function get_saved_documents($loan_id){
		$result =  $this->db->get_where('tbl_credit_scolair_documents',array('loan_id' => $loan_id))->row_array();
	...
	public function get_loan_details_branchwise()
	{
		...
		if($role_id == '2'){
		    // $this->db->select('g.loan_id,g.application_no,g.sub_product,g.customer_data,g.created_at,g.modified_at,b.branch_name,g.customer_type');
...
// 		}
		...
		return $result ;
		 ...
		 $details  =  array();
		    ...
		    $this->db->where_in('loan_type','credit_scolair');
    	...
    	//	echo $this->db->last_query(); die;
			...
			foreach($result as $res)
		    {
    		    ...
    		    if($user_data['department']){
    			    $bureau = $this->Common_model->get_bureau_value($user_data['department']);
    			    $this->db->where('g.department',$bureau);
    		    }
    			
    			if($role_id == '3')
    			{
    			    $this->db->where('g.loan_id',$res['loan_id']);
    			    $this->db->where('g.branch_id',$user_data['branch_id']);
    			}
    			else {
    			    $this->db->where('g.loan_id',$res['loan_id']);
    			}
        		$credit_scolair =  $this->db->get()->row_array();
		  ...
		  //  print_r($details); die;
    		...
    		$this->db->join('tbl_branch as b','b.bid = g.branch_id','left');
    		$this->db->join('tbl_temp_consumer_applicationform as loan','loan.aid = g.loan_id_temp','left');
			$swhere1 = "g.status is NULL";
			$this->db->where($swhere1);
			$this->db->where('g.deleted','0');
			
			if(!empty($loan_id)){
			    	$this->db->where_not_in('g.loan_id',$loan_id);
			}
			$this->db->order_by('g.loan_id','desc');
		
			
			$credit_scolair1 =  $this->db->get()->result_array();
			
			$result = array_merge($details,	$credit_scolair1);
...
// 		return $result  =  $this->db->get_where('tbl_all_applications',array('loan_id' => $loan_id,'loan_type' => $type,'review' => 1))->result_array();
	...
	public function get_risk_analysis_details($loan_id){
		...
		$result =  $this->db->get_where('tbl_credit_scolair_risk_analysis',array('loan_id' =>$loan_id))->row_array();
		return $result;
...
// 	{
// 		$result =  $this->db->get_where('tbl_credit_scolair_risk_analysis_facility',array('loan_id' =>$loan_id))->row_array();
// 		return $result;
// 	}
	...
	public function get_tracking_details($loan_id)
	{
		...
		$this->db->join('tbl_credit_scolair_applicationloan as loan','loan.loan_id = track.loan_id','inner');
	...
	public function get_number_of_loans($id){
			...
			$num_rows = $this->db->count_all_results('tbl_credit_scolair_applicationloan');
			return $num_rows;
			//echo  $this->db->last_query();
		...
		public function get_archive_loans()
	{
	    ...
	    $loan = $this->db->get_where('tbl_credit_scolair_applicationloan',$l_where)->row_array();
