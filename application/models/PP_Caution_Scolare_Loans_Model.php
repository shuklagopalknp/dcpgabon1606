...
class PP_Caution_Scolare_Loans_Model extends CI_Model
{
	...
	public function check_amortization_files($id){		
		if($id) {
			$query=$this->db->select('*')
			->from('temp_applicationform_caution_scolaire')
			->where('temp_id', $id)
			->where('loan_type',4)
			->get();
			//echo $this->db->last_query();			
	...
	public function get_customer_application_form($customerid)
	{
		...
		$query=$this->db->select('a.*,
			...
			->where('a.deleted', 0)
			->where('a.loan_type', 4)
			->where('a.customar_id',$customerid)
			//->where('a.admin_id', $this->session->userdata('id'))
			->order_by("a.modified desc")							
			->get();			
			return $query->result_array();
	...
	{
		$query=$this->db->select('*')
			->from('tbl_temp_applicationform_caution_scolaire')			
			->where('temp_id', $id)	
			->limit(1)			
			->get();			
			return $query->result_array();
	...
	## Student Information
	#
	public function get_stInformation($cid){
		$query=$this->db->select('*')
		->from('customer_student_information')
		->where('customar_id', $cid)
		->get();
		return $query->result_array();
	}
	...
	## Account Information
	#
	public function get_acInformation($cid){
		$query=$this->db->select('*')
	...
	#
	public function get_baInformation($cid){
		$query=$this->db->select('*')
		->from('customer_blocked_account_information')
		->where('customar_id', $cid)
		->get();
		return $query->result_array();
	}
	...
	public function get_caution_scolaire_loans_applicationform_details($id){		
		$query=$this->db->select('
			a.loan_type as loantype,
			a.*,
			u.first_name,
	...
	public function get_customerdetails_list(){
		$query=$this->db->select('c.*,
			...
			->join('customer_personalinformation cp','c.cid = cp.customar_id','inner')
			->join('caution_scolaire_loans_applicationform cl','c.cid = cl.customar_id','inner')
			->where('c.status', 1)
	...
	{
		$query=$this->db->select('*')
			->from('tbl_caution_scolaire_loans_applicationform')						
			->where('deleted', 0)
			->where('cl_aid',$id)								
			->get();			
			return $query->result_array();
	...
	public function filter_loan_statuslist($params)
	{
		if($params!="All")
		{
			$this->db->select(
