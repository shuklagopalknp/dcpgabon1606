...
class Escompte_Model extends CI_Model
{
	...
	public function save_loan_application($customer_id,$sub_product,$customer_type){
		...
		$product_tab =  $this->db->get_where('tbl_details_tabs_business_products',array('status' => '1','product_type' => 'escompte'))->result_array();
		...
		$system_docs  =  $this->db->get_where('tbl_document_system',array('document_status' => '1','product_type' => 'escompte','document_type' => "system_docs",'sub_product_type' => $sub_product))->result_array();
		...
		$checklist_docs  =  $this->db->get_where('tbl_document_system',array('document_status' => '1','product_type' => 'escompte','document_type' => "checklist_docs",'sub_product_type' => $sub_product))->result_array();
		...
		$risk_analysis_docs  =  $this->db->get_where('tbl_document_system',array('document_status' => '1','product_type' => 'escompte','document_type' => "risk_analysis_docs",'sub_product_type' => $sub_product))->result_array();
		...
		if($insert_id =  $this->Common_model->insertRow('tbl_escompte_applicationloan_n',$insertdata))
		{
			...
			$this->Common_model->insertRow('tbl_escompte_documents_n',$doc_data);
	...
	public function get_saved_documents($loan_id){
		$result =  $this->db->get_where('tbl_escompte_documents_n',array('loan_id' => $loan_id))->row_array();
	...
	public function get_loan_details_branchwise()
	{
        ...
        $this->db->select('e.loan_id,e.application_no,e.sub_product,e.customer_data,e.created_at,e.final_status,e.modified_at,b.branch_name,e.customer_type,app.workflow_order,app.assigned_roles,app.review,app.approval_status');
        $this->db->from('tbl_escompte_applicationloan_n as e');
        $this->db->join('tbl_branch as b','b.bid = e.branch_id','inner');
        $this->db->join('tbl_all_applications as app','app.loan_id =  e.loan_id','inner');
        if($role_id == '2' || $role_id == '3')
        {
                $this->db->where('e.branch_id',$user_data['branch_id']);
        }

        $this->db->where('e.deleted','0');
        $this->db->where('app.assigned_roles',$role_id);
        $this->db->where('app.status','1');
        $this->db->where('app.loan_type',"escompte");
        $this->db->where('app.review !=',NULL);
        $this->db->order_by('e.loan_id','desc');
	...
	public function get_single_loan_details($loan_id)
	{
        $role_id  =  $this->session->userdata('role');
            ...
            $this->db->select('tbl_escompte_applicationloan_n.*,app.workflow_order,app.assigned_roles,app.review,app.approval_status');
            $this->db->join('tbl_all_applications as app','app.loan_id =  tbl_escompte_applicationloan_n.loan_id','inner');
            $this->db->where('app.loan_type','escompte');
            $result = $this->db->get_where('tbl_escompte_applicationloan_n',array('tbl_escompte_applicationloan_n.loan_id' => $loan_id,'app.assigned_roles' => $role_id,'app.status'=> "1"))->row_array();
	...
	public function get_risk_analysis_details($loan_id){
		...
		$result['details'] =  $this->db->get_where('tbl_escompte_riskcalculation_n',array('loan_id' =>$loan_id))->row_array();
		$result['invoice_data'] =  $this->db->get_where('tbl_escompte_invoices_n',array('risk_calculation_id' =>$result['details']['id']))->result_array();
		return $result;
	...
	public function get_tracking_details($loan_id)
	{
		...
		$this->db->join('tbl_escompte_applicationloan_n as loan','loan.loan_id = track.loan_id','inner');
		$this->db->join('tbl_user as user','user.id =  track.user_id','inner');
		$this->db->where('track.loan_id',$loan_id);
                 $this->db->where('track.product_type','escompte');
	...
	public function get_interaction_history($loan_id){
		$result['email']  =  $this->db->get_where('tbl_business_interaction_history',array('loan_id' => $loan_id,'type' => "escompte",'mode' =>  "email"))->result_array();

		$result['sms']  =  $this->db->get_where('tbl_business_interaction_history',array('loan_id' => $loan_id,'type' => "escompte",'mode' =>  "sms"))->result_array();

		$result['interview']  =  $this->db->get_where('tbl_business_interaction_history',array('loan_id' => $loan_id,'type' => "escompte",'mode' =>  "interview_telephone"))->result_array();
