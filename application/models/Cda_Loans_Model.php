...
class Cda_Loans_Model extends CI_Model
{
	...
	public function countRow_LoansApplied()
	{
    	...
    	$this->db->where('cad_status !=', 'Initiated');
    	//$query=$this->db->get();
	...
	{
    	$this->db->from('customar_applicationform');
    	$this->db->where('cad_status', 'Confirm Disbursement');
    	//$query=$this->db->get();
    	return $num_rows = $this->db->count_all_results();
	}
	...
	{
    	$this->db->from('customar_applicationform');
    	$this->db->where('cad_status', 'Avis défavorable');
    	//$query=$this->db->get();
	...
	{
    	$this->db->from('customar_applicationform');
    	$this->db->where('cad_status', 'Process');
    	//$this->db->or_where('cad_status', 'Avis Favorable');
    	//$query=$this->db->get();
    	return $num_rows = $this->db->count_all_results();
	}
	...
	if($params!="All"){
			...
			->where('a.loan_type', 2);
			if($params != '') {
				$this->db->like('a.cad_status', $params);
			}
	...
	public function get_new_application_form()
	{
		...
		$maxvalue='10000000';
			...
			LEFT JOIN `tbl_amortization` `at` ON `a`.`amortization_id` = `at`.`id` 
			LEFT JOIN `tbl_customer_personalinformation` `pi` ON `a`.`customar_id` = `pi`.`customar_id` 
			WHERE `a`.`deleted` = 0 AND `a`.`loan_type` ='2' AND `a`.`cad_status` !='Initiated'
...
######################################################*/
	...
	public function get_decovert_application_form()
	{
		...
		$maxvalue='10000000';
			...
			WHERE `a`.`deleted` = 0 AND `a`.`loan_type` ='3' AND `a`.`cad_status` !='Initiated'
