...
(function($) {
    ...
    WizardCard.prototype = {
        ...
        validate: function() {
            ...
            this.el.find("[data-validate]").each(function(i, el) {
                ...
                if (!ret.status) {
                    failures = true;
                    el.parent(".form-control").toggleClass("error", true);
                    self.wizard.errorPopover(el, ret.msg);
                } else {
                    el.parent(".form-control").toggleClass("error", false);
                    try {
                        el.popover("destroy");
                    } catch (e) {
                        el.popover("hide");
 ...
 is", failures);
    ...
    Wizard = function(markup, args) {
        var wizard_template = ['<div class="modal wizard-modal" role="dialog">', '<div class="wizard-modal-header modal-header">', '<button class="wizard-close close" type="button">x</button>', '<h3 class="wizard-title"></h3>', '<span class="wizard-subtitle"></span>', '</div>', '<div class="pull-left wizard-steps">', '<div class="wizard-nav-container">', '<ul class="nav nav-list" style="padding-bottom:30px;">', '</ul>', '</div>', '<div class="wizard-progress-container">', '<div class="progress progress-striped">', '<div class="progress-bar progress-bar-primary" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100">', '<span class="sr-only">0% Complete</span>', '</div>', '</div>', '</div>', '</div>', '<form>', '<div class="wizard-cards">', '<div class="wizard-card-container">', '</div>', '<div class="wizard-modal-footer">', '<div class="wizard-buttons-container">', '<button class="btn wizard-cancel wizard-close" type="button">ANNULER</button>', '<div class="btn-group-single pull-right">', '<button class="btn wizard-back" type="button">PRECEDENT</button>', '<button class="btn btn-primary wizard-next" type="button">SUIVANT</button>', '</div>', '</div>', '</div>', '</div>', '</form>', '</div>'];
        this.args = {
            submitUrl: "",
            width: 750,
            showCancel: false,
        ...
        this.el.find(".wizard-steps").on("click", "li.already-visited a.wizard-nav-link", this, function(event) {
